class EmailTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @email_type.save
        format.html { redirect_to(@email_type, :notice => 'EmailType was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @email_type.update_attributes(params[:email_type])
        format.html { redirect_to(@email_type, :notice => 'EmailType was successfully updated.') }
        format.xml  { head :ok }
      else
